...
import com.qf.dao.ProductImgDao;
...
public class ProductServiceImpl implements ProductService {
    ...
    //2。网tb_product写入商品信息，获取priductID
    //3。结合productId批量处理商品详情页
    ...
    @Transactional
    //1.诺缩略图参数有值，则处理缩略图
    //，诺原先存在缩略图则先删除后添加新图，之后获取缩略图相对路径并赋值给product
    //2.诺商品详情列表参数有值，对商品详情图列表进行同样对操作
    //3.将tb_product_img下面的该商品原先的商品详情图记录全部清除
    //4。更新tb_product以及tb_prduct_img的信息
    ...
    private void deleteProductImgList(long productId) {
        ...
        //删除数据库里的图片信息
        productImgDao.deleteProductImgByPorduct(productId);
